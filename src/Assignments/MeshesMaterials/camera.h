...
public:
    ...
    void look_at(const glm::vec3& eye, const glm::vec3& center, const glm::vec3& up) {
        ...
        // Vectors defining orientation
        z_ = glm::normalize(eye - center);
        x_ = glm::normalize(glm::cross(up, z_));
        y_ = glm::normalize(glm::cross(z_, x_));

        position_ = eye; // Camera Position | Update
        center_ = center; // Center Point | Update
    ...
    void zoom(float y_offset) {
        std::cout << "Zoom applied." << std::endl;
        ...
        V_ = glm::lookAt(position_, center_, y_); // View Matrix | Update
    }
